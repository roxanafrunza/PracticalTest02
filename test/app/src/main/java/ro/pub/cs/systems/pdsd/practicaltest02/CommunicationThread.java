    ...
    public void run() {
        ...
        try {
            ...
            if (dataServer.containsKey(query1)) {
                ...
                String[] words = query1.split(",");
                HttpGet httpPost = new HttpGet("http://services.aonaware.com/CountCheatService/CountCheatService.asmx/LetterSolutionsMin?anagram=" + words[0] + "&minLetters=" + words[1]);
                ...
                Log.d("abc", query1);
                */
                List<Integer> indexesSt = new ArrayList<Integer>();
                List<Integer> indexesEnd = new ArrayList<Integer>();
                int st = pageSourceCode.indexOf("<string>");
                int end = pageSourceCode.indexOf("</string>");
                String querryData = "";

                String newPage = pageSourceCode.substring(end + "</string>".length());
                int index = 0;
                while(index != -1){
                    index = pageSourceCode.indexOf("<string>", index);
                    if (index != -1) {
                        indexesSt.add(index);
                        index++;
                    }
                ...
                while(index != -1){
                    index = pageSourceCode.indexOf("</string>", index);
                    if (index != -1) {
                        indexesEnd.add(index);
                        index++;
                    }
                }

                for(int i = 0; i < indexesSt.size(); i++)
                {
                    st = indexesSt.get(i);
                    end = indexesEnd.get(i);

                    querryData += " " + pageSourceCode.substring(st + "<string>".length(), end);
                }
